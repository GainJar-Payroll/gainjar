...
contract GainJar is Context, ReentrancyGuard, Ownable {
    ...
    function extendFiniteStream(address _employee, uint256 _additionalAmount, uint256 _additionalSeconds) external {
        ...
        uint256 streamableAmount = newRatePerSecond * newTotalTime;
        uint256 newFinalPayout = newTotalAmount - streamableAmount;
        ...
        // Use available balance for flow rate check
        uint256 availableBalance = getAvailableBalance(_msgSender());
    ...
    function withdraw(address _employer) external nonReentrant {
        ...
        uint256 amount = withdrawable(_employer, _msgSender());
        ...
        // Determine last withdrawal time
        ...
        // Mark as inactive if expired and fully withdrawn
        if (_isStreamExpired(stream) && stream.totalWithdrawn >= stream.totalAmount) {
            stream.isActive = false;
    ...
    /**
     * @notice Partial withdrawal - also fixed
     */
    ...
    function liquidate(address _employer) external nonReentrant {
        ...
        uint256 totalEmployeeEarnings = _calculateTotalEarnings(_employer, employees);
        ...
        uint256 totalPaid = _processLiquidationPayments(_employer, employees, vaultBalance, actualReward);

        // Pay liquidator reward
        if (actualReward > 0) {
            s_vaultBalances[_employer] -= actualReward;
            i_paymentToken.transfer(_msgSender(), actualReward);
        }

        emit Liquidated(_msgSender(), _employer, totalPaid, actualReward, employees.length);
    }
    ...
    function activateStream(address _employee) external {
        ...
        // Use available balance instead of total vault balance
        uint256 availableBalance = getAvailableBalance(_msgSender());
        uint256 minRequiredBalance = newFlowRate * MIN_COVERAGE_DAYS_SECOND;
    ...
    /**
     * @notice Get list of employees for a given employer
     * @param _employer Employer address
     */
    function getEmployeeList(address _employer) external view returns (address[] memory) {
    ...
    /**
     * @notice Get available balance (vault balance minus pending withdrawals)
     * @dev This is the ACTUAL available balance, not just vault balance
     * @param _employer Employer address
     * @return Available balance in wei
     */
    function getAvailableBalance(address _employer) public view returns (uint256) {
        uint256 vaultBalance = s_vaultBalances[_employer];

        // Calculate total earned but not withdrawn by ALL employees
        ...
        // Available = vault - pending withdrawals
        ...
        )
    {
        ...
        vaultAfterLiquidation =
            s_vaultBalances[_employer] > totalRequired ? s_vaultBalances[_employer] - totalRequired : 0;
    }
    ...
    {
        balance = getAvailableBalance(_employer);
        ...
        returns (uint256 totalEarned, uint256 safeAmount, bool isFullySafe)
    {
        ...
        uint256 available = getAvailableBalance(_employer);

        if (available >= totalEarned) {
            // Vault has enough available balance, fully safe
            safeAmount = totalEarned;
            isFullySafe = true;
        } else {
            // Vault doesn't have enough available funds, can only withdraw what's available
    ...
    function withdrawable(address _employer, address _employee) public view returns (uint256 earned) {
        ...
        // Only return 0 if stream never existed
        if (stream.startTime == 0) return 0;

        uint256 earnUntil = block.timestamp;

        // Cap at end time for finite streams
        if (stream.streamType == StreamType.FINITE && stream.endTime > 0 && earnUntil > stream.endTime) {
            earnUntil = stream.endTime;
        }
        ...
        // Cap at remaining budget for finite streams
        if (stream.streamType == StreamType.FINITE) {
    ...
    function getVaultDepletionTime(address _employer) public view returns (uint256 secondsUntilEmpty) {
        uint256 availableBalance = getAvailableBalance(_employer);
        uint256 flowRate = getTotalFlowRate(_employer);

        if (flowRate == 0) return type(uint256).max;

        secondsUntilEmpty = availableBalance / flowRate;
    }
    ...
    function _calculateFutureVaultStatus(address _employer, uint256 _newFlowRate) internal view returns (VaultStatus) {
        ...
        uint256 secondsUntilEmpty = availableBalance / _newFlowRate;
        uint256 daysRemaining = secondsUntilEmpty / 1 days;
    ...
    function _createStream(address _employee, uint256 _amount, uint256 _durationInSeconds, StreamType _type) internal {
        ...
        if (availableBalance < minRequiredBalance) revert GainJar__InsufficientEmployerVault(_msgSender());
        ...
        if (_type == StreamType.FINITE) {
            uint256 streamableAmount = ratePerSecond * _durationInSeconds;
            finalPayout = _amount - streamableAmount;
        }
        ...
        // Only add to employee list if not already there
        if (!s_employeeExist[_msgSender()][_employee]) {
        ...
        emit StreamCreated(
            _msgSender(), _employee, ratePerSecond, block.timestamp, endTime, _amount, _type, finalPayout
        );
    }
    ...
    /**
     * @notice Calculate total earnings for all active employees
     */
    function _calculateTotalEarnings(address _employer, address[] memory _employees)
        internal
        view
        returns (uint256 totalEarnings)
    {
        for (uint256 i = 0; i < _employees.length; i++) {
            Stream memory stream = s_streams[_employer][_employees[i]];
            if (stream.isActive && !_isStreamExpired(stream)) {
                totalEarnings += withdrawable(_employer, _employees[i]);
            }
        }
    }

    /**
     * @notice Calculate actual liquidator reward based on available vault
     */
    function _calculateActualReward(uint256 _vaultBalance, uint256 _totalEmployeeEarnings, uint256 _expectedReward)
        internal
        pure
        returns (uint256 actualReward)
    {
        uint256 totalRequired = _totalEmployeeEarnings + _expectedReward;

        if (_vaultBalance < totalRequired) {
            // Priority: Pay employees first
            if (_vaultBalance >= _totalEmployeeEarnings) {
                // Can pay all employees, reduce reward
                actualReward = _vaultBalance - _totalEmployeeEarnings;
            } else {
                // Can't even pay all employees, no reward
                actualReward = 0;
            }
        } else {
            actualReward = _expectedReward;
        }
    }

    /**
     * @notice Process payments to all employees during liquidation
     */
    function _processLiquidationPayments(
        address _employer,
        address[] memory _employees,
        uint256 _vaultBalance,
        uint256 _actualReward
    ) internal returns (uint256 totalPaid) {
        for (uint256 i = 0; i < _employees.length; i++) {
            totalPaid += _processEmployeeLiquidation(_employer, _employees[i], _vaultBalance, _actualReward, totalPaid);
        }

        // Clear active employee list
        delete s_activeEmployeeList[_employer];
    }

    /**
     * @notice Process liquidation for a single employee
     */
    function _processEmployeeLiquidation(
        address _employer,
        address _employee,
        uint256 _vaultBalance,
        uint256 _actualReward,
        uint256 _totalPaidSoFar
        ...
        uint256 earned = withdrawable(_employer, _employee);

        if (earned > 0) {
            // Calculate maximum available for this employee
            uint256 maxAvailable = _vaultBalance - _actualReward - _totalPaidSoFar;
            uint256 payment = earned > maxAvailable ? maxAvailable : earned;

            if (payment > 0) {
                paid = _executeEmployeePayment(_employer, _employee, payment, stream);
            }
        }

        // Pause stream
        stream.isActive = false;
        emit StreamPaused(_employer, _employee);
    }

    /**
     * @notice Execute payment to employee
     */
    function _executeEmployeePayment(address _employer, address _employee, uint256 _payment, Stream storage _stream)
        internal
        returns (uint256 payment)
    {
        (uint256 fee, uint256 netAmount) = _calculateFee(_payment);

        _stream.lastWithdrawal = block.timestamp;
        _stream.totalWithdrawn += _payment;

        s_vaultBalances[_employer] -= _payment;

        i_paymentToken.transfer(_employee, netAmount);
        s_accumulatedFees += fee;

        emit Withdrawal(_employee, netAmount, fee);
