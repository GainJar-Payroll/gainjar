...
contract GainJarWithdrawSurplusTest is BaseTest {
    ...
    function test_WithdrawSurplus_WithActiveStream_AllowsWhenSafe() public {
        ...
        // withdrawing the extra should be allowed
        uint256 withdrawAmt = 100 * 1e6;
        ...
        // Allow up to 0.1 USDC difference due to per-second rounding
        assertApproxEqAbs(
            vaultBalance,
            600 * 1e6,
            0.1 * 1e6, // 0.1 USDC tolerance
            "vault left with min coverage"
        );
    }
    ...
    function test_RevertWhen_WithdrawSurplus_AmountZero() public {
        // Setup: 100 USDC per day stream -> min required for 7 days = 700 USDC
        uint256 ratePerPeriod = 100 * 1e6; // per day
        ...
        vm.expectRevert(abi.encodeWithSelector(GainJar.GainJar__InvalidWithdrawAmount.selector));
        vm.prank(employer);
        gainjar.withdrawSurplus(0);
    }
}
